...
const FeaturedTranscriptClient = ({
  ...
  const getSourceFromTranscript = (data: Transcript) =>
    SourceCountData.find((source) => source.slug === data.slugAsParams[0])
      ?.name ?? (data.slugAsParams as Array<string>)[0];
      ...
      <section className="flex flex-col gap-6 w-full">
        <section className="flex items-center justify-between w-full">
          <h3 className="text-2xl font-semibold max-md:text-xl">
            {t("home.dive.featured-transcripts")}
          </h3>
          <button
            className="w-fit px-5 py-3 rounded-full border border-black flex gap-1 items-center max-md:py-1.5 max-md:px-3"
            onClick={randomise}
          >
            <span>
              <DiceIcon className="w-5 text-black max-md:w-3" />
            </span>
            <p className="leading-[19.2px] text-sm font-medium max-sm:text-xs">
              {t("home.dive.randomize")}
            </p>
          </button>
        </section>
        <div className="grid auto-rows-max gap-5 max-sm:grid-cols-1 max-lg:grid-cols-2 max-xl:grid-cols-2 grid-cols-3">
          {featured.slice(0, 6).map((transcript, idx) => (
            <TranscriptCard
              data={transcript}
              key={idx}
              source={getSourceFromTranscript(transcript)}
              languageCode={languageCode}
            />
          ))}
        </div>
        <section className="flex flex-col gap-6 w-full">
          <h3 className="text-2xl font-semibold max-md:text-xl">
            {t("home.dive.latest-transcripts")}
          </h3>
          <div className="grid auto-rows-max gap-5 max-sm:grid-cols-1 max-lg:grid-cols-2 max-xl:grid-cols-2 grid-cols-3">
            {latestTranscripts.map((transcript, idx) => (
              <TranscriptCard
                data={transcript}
                daysOpened={transcript.days_opened}
                key={idx}
                source={getSourceFromTranscript(transcript)}
                languageCode={languageCode}
              />
